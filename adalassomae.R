adalassomae = function (X, y, k = 10, use.Gram = TRUE, both = TRUE, intercept = TRUE) 
{
  colnames(X) = 1:ncol(X)
  n <- length(y)
  cv.adalasso <- NULL
  ...
  if (both == TRUE) {
    ...
    for (i in seq(k)) { 
      ...
      ytest <- y[omit]
      my.lars <- mylars(Xtrain, ytrain, k = k-2, normalize = TRUE, 
                        use.Gram = use.Gram, intercept = intercept) # use k=k-2 to handle LOOCV
...
#       }
      if (length(weights) > 1) {
        ...
        residmat[, i] <- apply(abs(ytest - pred), 2, mean)
      }
    }
